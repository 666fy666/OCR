...
def draw_ocr(
    ...
    args:
        ...
        txts(list): the texts
        scores(list): texts corresponding scores
    ...
    # Create copies for the left and right parts
    image_left = image.copy()
    image_right = np.ones_like(image) * 255  # White canvas for the right part
    ...
    for i, box in enumerate(boxes):
        ...
        # Draw the detection box on the left image
        box = np.array(box).astype(np.int32).reshape((-1, 1, 2))
        cv2.polylines(image_left, [box], isClosed=True, color=(255, 0, 0), thickness=2)

        # Draw the recognized text on the right image
        if txts is not None:
            text = txts[i]
            # Calculate the bounding box for the text
            x_min, y_min = np.min(box[:, 0, :], axis=0)
            x_max, y_max = np.max(box[:, 0, :], axis=0)
            box_width = x_max - x_min
            box_height = y_max - y_min

            # Dynamically adjust font size to fit the box
            font_size = 20  # Start with a default font size
            pil_image = Image.fromarray(image_right)
            ...
            while True:
                ...
                # Use ImageDraw.textbbox to calculate text size
                bbox = draw.textbbox((0, 0), text, font=font)  # Get the bounding box of the text
                text_width = bbox[2] - bbox[0]
                text_height = bbox[3] - bbox[1]

                if text_width <= box_width and text_height <= box_height:
                    font_size += 1  # Increase font size if it still fits
                else:
                    font_size -= 1  # Reduce font size to fit the box
                    font = ImageFont.truetype(font_path, font_size)
                    break
            ...
            image_right = np.array(pil_image)
